...
class AuthenticationTests(APITestCase):
    ...
    def test_AUTH_03_warga_tidak_bisa_akses_halaman_admin(self):
        ...
        """
        # Login sebagai warga biasa melalui session Django
        ...
        self.assertTrue(
            login_success,
            "Warga biasa harus berhasil login terlebih dahulu"
        )

        # Warga biasa mencoba mengakses halaman dashboard admin
        response = self.client.get('/dashboard/')

        # Karena warga bukan admin/staff, akses harus ditolak atau diarahkan ulang
        self.assertIn(
            response.status_code,
