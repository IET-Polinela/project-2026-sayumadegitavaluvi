...
class WorkflowStateTests(APITestCase):
    ...
    def test_WF_02_tidak_bisa_edit_laporan_yang_sudah_reported(self):
        ...
        # Simpan data asli sebelum percobaan edit
        original_title = self.laporan_reported.title
        original_category = self.laporan_reported.category
        original_description = self.laporan_reported.description
        original_location = self.laporan_reported.location
        original_status = self.laporan_reported.status

        url = f'/api/report/{self.laporan_reported.pk}/'
        payload = {
            'title': 'Judul Ini Seharusnya Tidak Tersimpan',
        ...
        self.assertEqual(
            ...
            "Laporan berstatus REPORTED tidak boleh diedit oleh warga"
        )

        self.laporan_reported.refresh_from_db()

        self.assertEqual(self.laporan_reported.title, original_title)
        self.assertEqual(self.laporan_reported.category, original_category)
        self.assertEqual(self.laporan_reported.description, original_description)
        self.assertEqual(self.laporan_reported.location, original_location)
        self.assertEqual(self.laporan_reported.status, original_status)
    ...
    def test_WF_05_laporan_resolved_tidak_bisa_diubah(self):
        ...
        # Simpan data asli sebelum percobaan edit
        original_title = self.laporan_resolved.title
        original_category = self.laporan_resolved.category
        original_description = self.laporan_resolved.description
        original_location = self.laporan_resolved.location
        original_status = self.laporan_resolved.status

        url = f'/api/report/{self.laporan_resolved.pk}/'
        payload = {
            'title': 'Judul RESOLVED Diubah',
            'category': self.laporan_resolved.category,
            'description': 'Deskripsi RESOLVED ini seharusnya tidak berubah.',
        ...
        self.assertEqual(self.laporan_resolved.title, original_title)
        self.assertEqual(self.laporan_resolved.category, original_category)
        self.assertEqual(self.laporan_resolved.description, original_description)
        self.assertEqual(self.laporan_resolved.location, original_location)
        self.assertEqual(self.laporan_resolved.status, original_status)
...
class AdminWorkflowTests(TestCase):
    ...
    def test_WF_03_admin_mengubah_status_reported_ke_verified(self):
        ...
        self.assertIn(
            ...
            "Update status oleh admin seharusnya berhasil"
        )
    ...
    def test_WF_04_tidak_ada_transisi_langsung_ke_resolved_dari_reported(self):
        ...
        """
        from main_app.views import get_next_status_action

        next_action = get_next_status_action(self.laporan_reported.status)

        self.assertIsNotNone(
            next_action,
            "Laporan REPORTED seharusnya memiliki transisi lanjutan"
        )

        self.assertEqual(
            next_action['value'],
            'VERIFIED',
            "Transisi valid dari REPORTED hanya boleh menuju VERIFIED"
        )
