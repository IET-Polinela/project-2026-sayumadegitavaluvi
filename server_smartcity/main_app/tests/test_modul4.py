...
class CRUDAndValidationTests(APITestCase):
    ...
    def test_FT_01_buat_laporan_dengan_data_lengkap(self):
        ...
        payload = {
            ...
            'location': 'Gerbang Utama Polinela',
        }
        ...
        self.assertEqual(
            ...
            "Pembuatan laporan dengan data lengkap seharusnya berhasil"
        )

        laporan = Report.objects.get(title='Lampu Jalan Mati')

        self.assertEqual(laporan.category, 'Infrastruktur')
        self.assertEqual(laporan.description, 'Lampu jalan di depan gerbang kampus tidak menyala.')
        self.assertEqual(laporan.location, 'Gerbang Utama Polinela')
    ...
    def test_FT_02_ditolak_jika_judul_kosong(self):
        ...
        payload = {
            'category': 'Infrastruktur',
            'description': 'Deskripsi laporan tetap diisi, tetapi title tidak dikirim.',
        ...
        self.assertEqual(
            ...
            "Pembuatan laporan tanpa title seharusnya ditolak"
        )

        self.assertIn(
            'title',
            response.data,
            "Response error harus memuat validasi untuk field title"
        )
    ...
    def test_FT_03_ditolak_jika_deskripsi_kosong(self):
        ...
        payload = {
            'title': 'Air Tidak Mengalir',
            'category': 'Fasilitas Umum',
            'location': 'Toilet Gedung A',
        }
        ...
        self.assertEqual(
            response.status_code,
            status.HTTP_400_BAD_REQUEST,
            "Pembuatan laporan tanpa description seharusnya ditolak"
        )

        self.assertIn(
            'description',
            response.data,
            "Response error harus memuat validasi untuk field description"
        )
